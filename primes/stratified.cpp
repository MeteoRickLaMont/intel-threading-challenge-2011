...
//
// What makes this fast is to consider k-pairs with constant stride lengths
// (in k-pair terminology, stride = k - 1). At each iteration of the inner
// loop we're effectively adding one prime at the leading edge and subtracting
// one prime at the trailing edge. Thus the sum grows gradually, at a pace
// similar to that of the power table. The linear search of the power table
// in the inner loop usually iterates 0 or 1 times.
//
// The running time for this algorithm is O(n^2 + m). It is appropriate for
// all inputs because m tends to grow more rapidly than n^2.
//
void strategystride(int tnum)
{
    ...
    //
    // Split gNKPairs into gNSearchThreads equal portions.
    // The last portion may be slightly smaller due to rounding.
    //
    uint64_t chunk = (gNKPairs + gNSearchThreads - 1) / gNSearchThreads;
    uint32_t stride = firststride(tnum, gNSearchThreads, chunk) + 1;
    uint32_t nextstride = firststride(tnum + 1, gNSearchThreads, chunk) + 1;
    if (nextstride > gNSums)
        nextstride = gNSums;
    ...
    for (; stride < nextstride; ++stride) {
        ...
        if (p + BINARY_THRESHOLD <= gPowerEnd && p[BINARY_THRESHOLD].n < sum) {
            //
